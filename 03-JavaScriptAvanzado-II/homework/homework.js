...
function counter() {
  ...
  al realizar una operación por segunda vez, se pueda obtener el resultado de esa "memoria" sin tener que e
  fectuar otra vez cálculos que ya se hicieron anteriormente.
  ...
  que, la próxima vez que reciba el mismo argumento, no sea necesario volver a invocar a cb, porque el 
...
function cacheFunction(cb) {
  var obj = {};
  return function (arg) {
    if (obj.hasOwnProperty(arg)){ /*Para saber si un objeto tiene una propiedad utilizamos el método .hasOwnProperty, 
      el cual recibe como parámetro el nombre de la propiedad sobre la cuál queremos conocer su existencia 
      y devuelve un booleano , false o true.*/
      return obj [arg]; // retorna el argumento del objeto
    }else {
      obj [arg] = cb (arg); // asignamos el argumento al objeto
      return obj [arg]; // retorna el argumento del objeto
...
/*
  ...
  tres funciones que retornen una cadena (string) y el delimitador especificado (
    asteriscos, guiones, y guiones bajos, respectivamente). L
    as funciones obtenidas deberían recibir solamente un argumento - la cadena de texto - 
